...
path = r"c:\Users\SongYuanXinTong\Documents\trae_projects\器材装备管理小程序\backend\src\services\record.service.ts"
content = """
import db from '../db'
...
export function createRecord(input: CreateRecordInput, operatorId: number): Record {
  if (!VALID_TYPES.includes(input.type)) {
    throw new Error('type 必须为 in 或 out')
  }
  ...
  const create = db.transaction((): { record: Record; eq: { id: number; name: string } } => {
    ...
    if (input.type === 'out') {
      ...
      if (qty > stock) {
        throw new Error(`库存不足：当前库存 ${stock}，出库数量 ${qty}`)
      }
    }
    ...
    if (input.quantity !== undefined) {
      ...
      if (existing.type === 'out') {
        ...
        if (delta > 0 && delta > currentStock) {
          throw new Error(`库存不足：当前库存 ${currentStock}，需要增加出库数量 ${delta}`)
        }
      }
  ...
  db.transaction(() => {
    ...
    if (existing.type === 'in') {
      if (currentStock - existing.quantity < 0) {
        throw new Error(`删除后库存将为负数：当前库存 ${currentStock}，入库数量 ${existing.quantity}`)
      }
    }
...
export function attachPhotos(recordId: number, photos: AttachPhotoInput[]): RecordPhoto[] {
  ...
  for (const p of photos) {
    if (!VALID_PHOTO_KINDS.includes(p.kind)) {
      throw new Error('kind 非法，必须为 product/location/annotated/video')
    }
    if (!p.url) {
      throw new Error('照片 url 不能为空')
    }
  }
  ...
  const insert = db.transaction((): RecordPhoto[] => {
    ...
    if (existingNonVideo + newNonVideo > PHOTO_LIMIT) {
      throw new Error(`照片数量超过上限（最多 ${PHOTO_LIMIT} 张）`)
    }
    if (existingVideo + newVideo > VIDEO_LIMIT) {
      throw new Error(`视频数量超过上限（最多 ${VIDEO_LIMIT} 个）`)
    }
...
with open(path, "w", encoding="utf-8") as f:
    f.write(content)
print("done: record.service.ts", len(content), "bytes")
